...
# Pre-compute lowercase keys for faster lookups
_MODEL_PRICING_LOWER = {k.lower(): v for k, v in MODEL_PRICING.items()}
...
def calculate_api_cost(model_name: str, input_tokens: int, output_tokens: int) -> float:
	...
	pricing = _MODEL_PRICING_LOWER.get(model_lower)

	if pricing is None:
		logger.warning(f"No pricing found for model {model_name}. Returning cost of 0.0")
		return 0.0
